"""Phase 2 — role-specific tables for manager, practitioner, distributor
...
def upgrade() -> None:
    ...
    op.create_index("ix_practitioner_clients_practitioner", "practitioner_clients", ["practitioner_id"])
    op.create_index("ix_coaching_sessions_practitioner", "coaching_sessions", ["practitioner_id"])
    op.create_index("ix_coaching_sessions_scheduled", "coaching_sessions", ["scheduled_at"])
    op.create_index("ix_follow_ups_practitioner", "follow_ups", ["practitioner_id"])
    op.create_index("ix_follow_ups_due_date", "follow_ups", ["due_date"])
    op.create_index("ix_distributor_practitioners_dist", "distributor_practitioners", ["distributor_id"])
    op.create_index("ix_credit_transactions_dist", "credit_transactions", ["distributor_id"])


def downgrade() -> None:
    # Drop indexes
    for idx in [
        "ix_credit_transactions_dist", "ix_distributor_practitioners_dist",
        "ix_follow_ups_due_date", "ix_follow_ups_practitioner",
        "ix_coaching_sessions_scheduled", "ix_coaching_sessions_practitioner",
        "ix_practitioner_clients_practitioner",
        "ix_interviews_scheduled", "ix_interviews_interviewer",
        "ix_candidates_position", "ix_hiring_positions_manager",
        "ix_training_assignments_user", "ix_training_assignments_manager",
    ]:
        op.drop_index(idx)
